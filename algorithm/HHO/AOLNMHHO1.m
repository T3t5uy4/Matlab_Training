...
function [Rabbit_Energy,Rabbit_Location, CNVG]=AOLNMHHO1(N,MaxFES,lb,ub,dim,fobj)
...
if Boundary_size > 1
    for i = 1:N
        for j = 1:dim
            OXP(i,j) = rand() * (ub(j) + lb(j)) - X(i,j);
            if OXP(i,j) > ub(j) || OXP(i,j) < lb(j)
                OXP(i,j) = rand() * (ub(j) - lb(j)) + lb(j);
            end
        end
    end
else
    for i = 1:N
        OXP(i,:) = rand() * (ub + lb) - X(i,:);
        for j = 1:dim
            if OXP(i,j) > ub || OXP(i,j) < lb
                OXP(i,j) = rand() * (ub - lb) + lb;
            end
        end
    end
end
for i = 1:N
    OXPfitness(i) = fobj(OXP(i,:));
    FES = FES + 1;
    if OXPfitness(i) < Rabbit_Energy
        Rabbit_Energy = OXPfitness(i);
        Rabbit_Location = OXP(i,:);
    end
    fitcount = fitcount + 1;
    CNVG(1,fitcount) = Rabbit_Energy;
end
allPositions = [X;OXP];
allFitness = [Pfitness OXPfitness];
...
while FES<MaxFES
    ...
    c = 2 * sqrt(1 - (FES / MaxFES)^2); % factor to show the decreaing energy of rabbit

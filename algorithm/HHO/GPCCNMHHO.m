...
function [Rabbit_Energy,Rabbit_Location, CNVG]=GPCCNMHHO(N,MaxFES,lb,ub,dim,fobj)
...
%                 FES=FES+2;
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%引入纵向交叉%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        [fitness_mvc(i),fitness_x(i),Mvc(i,:),X(i,:),Rabbit_Location,Rabbit_Energy,gbestfit,FES,count] = Cross_Energy(X(i,:),Rabbit_Location,Rabbit_Energy,lb,ub,dim,FES,fobj);
        CNVG(:,fitcount + 1:fitcount + count) = gbestfit;
    ...
    for i = 1:size(X,1)
        X(i,:) = X_mix(index(i),:);
        allFitness(i) = fitness_mix(i);
    end
    if allFitness(1) < Rabbit_Energy
        Rabbit_Energy = allFitness(1);
        Rabbit_Location = X(1,:);
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%引入横向交叉%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [Mhc,fitness_mhc,X,Rabbit_Location,Rabbit_Energy,gbestfit,FES,count]= Criss_Energy(X,Rabbit_Location,Rabbit_Energy,lb,ub,dim,FES,fobj);
    CNVG(:,fitcount + 1:fitcount + count) = gbestfit;
    fitcount = fitcount + count;
    fitness_mix = [allFitness;fitness_mhc];
    X_mix = [X;Mhc];
    [fitness_mix,index] = sort(fitness_mix,'ascend');
    for i =1:size(X,1)
        X(i,:) = X_mix(index(i),:);

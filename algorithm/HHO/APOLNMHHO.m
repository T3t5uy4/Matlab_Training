...
function [Rabbit_Energy,Rabbit_Location, CNVG]=APOLNMHHO(N,MaxFES,lb,ub,dim,fobj)
...
for i=1:M
    if Boundary_size == 1
        S(i,:)=rand()*(ub+lb)-X2(i,:);
        for j=1:dim
            if S(i,j)>ub || S(i,j)<lb
                S(i,j)=rand()*(ub(j)-lb(j))+lb(j);
            end
        end
    else
        for j = 1:dim
            S(i,j)=rand()*(ub(j)+lb(j))-X2(i,j);
            if S(i,j)>ub(j) || S(i,j)<lb(j)
                S(i,j)=rand()*(ub(j)-lb(j))+lb(j);
            end
        end 
    end
    Sfitness(i)=fobj(S(i,:));
    FES=FES+1;
    if Sfitness(i)<Rabbit_Energy
        Rabbit_Energy=Sfitness(i);
        Rabbit_Location = S(i,:);
    end
    fitcount=fitcount+1;
    CNVG(1,fitcount)=Rabbit_Energy;   
...
AllPositions=[X; S];
allfitness=[Pfitness Sfitness];
[~,index]=sort(allfitness);
X=AllPositions(index(1:SearchAgents_no),:);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

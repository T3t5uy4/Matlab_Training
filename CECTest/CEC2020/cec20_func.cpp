...
void cec20_test_func(double* x, double* f, int nx, int mx, int func_num0) {
	...
	if(ini_flag==0) {
		...
		/* Load Matrix M*/
		sprintf(FileName, "CECTest/CEC2020/input_data/M_%d_D%d.txt", func_num, nx);
		...
		/* Load shift_data */
		sprintf(FileName, "CECTest/CEC2020/input_data/shift_data_%d.txt", func_num);
		fpt = fopen(FileName, "r");
		if(fpt==NULL) {
		...
		{
			sprintf(FileName, "CECTest/CEC2020/input_data/shuffle_data_%d_D%d.txt", func_num, nx);
			fpt = fopen(FileName, "r");
			if(fpt==NULL) {
